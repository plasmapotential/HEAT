...
def inputDragDrop(file, contents, MachFlag):
    ...
    if file is None:
        ...
        outputDiv = dbc.Label("Loaded Input File", style={'color':'#f5d142'})
...
#                            children=[
#                                dbc.Label("Width (px)"),
#                                dbc.Input(id="EQpixelX"),
#                            ],
#                        ),
#                        html.Div(
#                            children=[
#                                dbc.Label("Height (px)"),
#                                dbc.Input(id="EQpixelY"),
#                            ],
#                        ),
#                        dbc.Button("Save EQs to .png", id="saveEQbutton", n_clicks=0, color="secondary"),
#                    ],
#                ),
...
def gfileUpload(gFile, MachFlag):
    if MachFlag is None:
        ...
        if len(gFile) > 1:
            div = dbc.Label("Loaded multiple gfiles", style={'color':'#f5d142'})
        else:
            div = dbc.Label("Loaded gFile: "+gFile[0], style={'color':'#f5d142'})
        return [div]
...
def loadRes(n_clicks, gridRes, MachFlag):
    ...
    return [dbc.Label("Loaded Resolution Settings", style={'color':'#f5d142'}), CADdata]
...
def loadCAD(STPfile, ts, STPcontents, MachFlag):
    ...
    return [dbc.Label("Loaded CAD: "+STPfile[0], style={'color':'#f5d142'})]
...
def velocityFromFile(className):
    return html.Div(
        children=[
            dbc.Label("3D plasma temperature interpolation not yet available", style={'color':'#f5d142'})
        ],
        className = className
    )
...
def allROISource(className):
    return html.Div(
        children=[
            dbc.Label("Using all ROI PFCs as gyro power sources", style={'color':'#f5d142'})
        ],
        className = className
    )
...
def loadGYRO(n_clicks,N_gyroSteps,N_gyroPhase,gyroDeg,ionMassAMU,vMode,gyroT_eV,
    ...
    return [dbc.Label("Loaded Gyro Orbit Data into HEAT", style={'color':'#f5d142'}),
            gyroPhaseFig,
            vPhaseFig,
            vSliceFig,
            cdfSliceFig,
...
def loadRAD(n_clicks,phiMin,phiMax,Ntor,Nref,radFile,radData):
    ...
    if radFile == None:
        ...
        outDiv = dbc.Label("Loaded RAD Data into HEAT", style={'color':'#f5d142'})
        Ntor = int(Ntor)
        Nref = int(Nref)
        phiMin = float(phiMin)
        phiMax = int(phiMax)
...
def gfileUpload(radFile, MachFlag):
    if MachFlag is None:
        ...
        return [dbc.Label("Loaded radFile: "+radFile[0], style={'color':'#f5d142'})]
...
def loadOF(n_clicks,OFstartTime,OFstopTime,
    ...
    if n_clicks < 1:
        ...
        return [dbc.Label("Loaded OF Data into HEAT", style={'color':'#f5d142'}), OFdata]
    ...
    return html.Div(
                children=[
                    html.Div(
                        children=[
                            dbc.Label("x [mm]"),
                            dbc.Input(id="xGyroTrace", className="xyzBoxInput"),
                            dbc.Label("y [mm]"),
                            dbc.Input(id="yGyroTrace", className="xyzBoxInput"),
                            dbc.Label("z [mm]"),
                            dbc.Input(id="zGyroTrace", className="xyzBoxInput"),
                        ],
                        className="xyzBox"
                    ),
                    html.Div(
                        children=[
                            dbc.Label("Temperature [eV]"),
                            dbc.Input(id="gyroT_eV_trace", className="xyzBoxInput"),
                            dbc.Label("Gyro Phase [deg]"),
                            dbc.Input(id="gyroPhase_trace", className="xyzBoxInput"),
                            dbc.Label("Trace Length [deg]"),
                            dbc.Input(id="gyroDeg_trace", className="xyzBoxInput"),
                            dbc.Label("# Helix Discretizations"),
                            dbc.Input(id="N_gyroSteps_trace", className="xyzBoxInput"),
                            dbc.Label("Trace Direction"),
                            dbc.Input(id="gyroDir_trace", className="xyzBoxInput"),
                        ],
                        className="xyzBox"
                    )
                    ],
...
def interpolate(n_clicks, t):
    ...
    return [dbc.Label("Gfile Interpolated", style={'color':'#f5d142'}),
            dcc.send_file(name)]
...
def session_data(n_clicks, inputTs, ts, MachFlag, data, inputFileData):
    ...
    if btnTest and (MachFlag is not None):
        ...
        defaultDiv = [dbc.Label("Loaded Defaults", style={'color':'#f5d142'})]
