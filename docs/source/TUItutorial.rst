...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
To run HEAT in terminal mode, point the ``run.sh`` wrapper (repo root) at your
batchFile — the folder containing it is mounted at /root/terminal inside the
container::

  ./run.sh tui <batchModePath>/batchFile.dat

Alternatively, launch a bash shell inside the container and run HEAT manually::

  ./run.sh shell

The HEAT image uses an ENTRYPOINT to run HEAT by default, so a shell requires
overriding the entrypoint; ``run.sh shell`` does this for you (the raw
equivalent is ``docker compose run --rm --entrypoint "" HEAT /bin/bash`` from
the docker directory).  Run ``./docker/setup.sh`` once first so the container
gets the correct user id (UID) and group id (GID) from the host.

Mapping host directories into the container
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
One benefit of docker is that you can bind mount directories from your host
machine directly into the container.  The docker-compose.yml recipe is
parameterized via ``docker/.env`` (generated by ``./docker/setup.sh``), so the
standard mounts are configured there rather than by editing the recipe:

 - **HEAT data directory** — ``HEAT_DATA_DIR`` in docker/.env (default
   ``~/HEAT``) is mounted at ``/root/HEAT``.
